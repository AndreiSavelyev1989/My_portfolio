...
export const ContactInfo: React.FC<PropsType> = (props) => {
    return (
        <div className={style.contactInfoBlock}>
            <div className={style.contactInfo}>
                <h4 className={style.contentTitle}>Contact Info</h4>
                <p className={style.infoDescription}>Always available for freelance work if the right
                    project comes along, Feel free to contact me!</p>
                {props.contactInfo.map(c => <PersonalInfo
                    key={c.id}
                    title={c.title}
                    icon={c.icon}
                    content={c.content}/>)}
